...
let AuthService = class AuthService {
    ...
    async changeCredentials(adminId, data) {
        ...
        if (!ok)
            throw new common_1.UnauthorizedException('Invalid credentials');
        const username = data.username !== undefined && data.username.trim().length > 0
            ? data.username.trim()
            : admin.username;
        const password = data.password !== undefined && data.password.trim().length > 0
            ? await bcrypt.hash(data.password, 10)
            : admin.password;
        const name = data.name !== undefined && data.name.trim().length > 0
            ? data.name.trim()
            : admin.name;
